...
def parse_sig_loglines_to_trades(log_lines):
    ...
    for line in (log_lines or []):
        ...
        if sig_type in ("BUY", "SELL_SHORT"):
            ...
            # Add mapping for EXIT_ALL_PARTIALS
            kind = sig_type
            if kind == "EXIT_ALL_PARTIALS":
                kv["exit_cause"] = "ALL_PARTIALS"
...
class StrategyV13(StrategyBase):
    def __init__(self, *args, **kwargs):
        ...
        self._last_entry_tier: Dict[str, str] = {}          # symbol -> 'A' or 'B'
        self._last_entry_time = {}
        self._entry_price_by_symbol = {}
        cfg = getattr(self, "config", {}) or {}
        raw = getattr(self, "cfg_d", {}).get("fixed_sl_pct_by_tier", {}) or {}
        self.fixed_sl_pct_by_tier: Dict[str, float] = {str(k).upper(): float(v) for k, v in raw.items()}
        self.fixed_sl_pct_global = float(getattr(self, "cfg_d", {}).get("fixed_sl_pct", 0.10))


    def get_sl_pct_for_tier(self, tier) -> float:
        tid = self._as_tier_id(tier) or "A"
        if isinstance(self.fixed_sl_pct_by_tier, dict) and tid in self.fixed_sl_pct_by_tier:
            return float(self.fixed_sl_pct_by_tier[tid])
        if isinstance(self.fixed_sl_pct_by_tier, dict) and "*" in self.fixed_sl_pct_by_tier:
            return float(self.fixed_sl_pct_by_tier["*"])
        return float(getattr(self, "fixed_sl_pct_global", getattr(self, "fixed_sl_pct", 0.10)))

    def _as_tier_id(self, tier) -> str | None:
        """
        Normalize to 'A' or 'B'. Accepts 'A'/'B', 1/2, or dicts like {'tier': 'A'}.
        """
        if tier is None:
            return None
        if isinstance(tier, dict):
            tier = tier.get("tier") or tier.get("id")
    ...
    def _compute_position_size(self, symbol: str, entry: float, stop: float, direction: int) -> float:
        """
        ...
        - Min notional guard
        NOTE: Uses per-symbol tier from self._last_entry_tier if available.
        """
        cfg = getattr(self, "cfg_d", {}) or {}
        tier = None
        if isinstance(getattr(self, "_last_entry_tier", None), dict):
            tier = self._last_entry_tier.get(symbol)
    ...
    def _apply_ratchet(self, side: str, price: float, prev_stop: float, entry: float,
                       trigger_move_pct: float, cushion_pct: float, sym=None, ts=None):
        """
        Move stop only in your favor.
          LONG:  stop = max(prev_stop, price * (1 - cushion_pct))
          SHORT: stop = min(prev_stop, price * (1 + cushion_pct))
        """
        cushion_abs = price * cushion_pct
        if side.upper() == "LONG":
            target = price - cushion_abs
            new_stop = max(prev_stop, target)
        else:  # SHORT
            target = price + cushion_abs
            new_stop = min(prev_stop, target)
        if new_stop != prev_stop:
            self._dbg(sym or "?", {"timestamp": ts or "?", "close": price},
                      tag="STOP_UPDATE",
                      extra={"from": f"{prev_stop:.4f}", "to": f"{new_stop:.4f}",
    ...
    def _maybe_do_partials(self, ts, price, trade):
        ...
        for level_idx, lvl in enumerate(self.partial_levels):
            ...
            if remaining <= 0:
                # Emit a terminal exit event so the parser knows this trade ended via partials
                if remaining <= 1e-9:
                    self._dbg(trade.get("symbol", "?"),
                              f"EXIT_ALL_PARTIALS | rem=0 | reason=ALL_PARTIALS | last_fill={fill_px}",
                              ts=ts, fill=fill_px)
                break
    ...
    def on_end(self, *_, **__):
        ...
        dump_dir = getattr(self, "results_dump_path", None) or "./logs/results"
    ...
    def _tier_for_context(self, sym, bar, ema9, ema20, vwap, macd, sig):
        ...
        # region (not chasing too far past VWAP), allow Tier B. Otherwise Tier A.
