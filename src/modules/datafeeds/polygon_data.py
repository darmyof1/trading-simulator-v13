# src/modules/datafeeds/polygon_data.py
...
def premarket(symbols, date_str, outdir, label):
    print(f"[POLYGON] (stub) premarket {date_str} {symbols} → {outdir}/{label}_*")

def stream(symbols, date_str, outdir, label, ingress: IngressMode):
    print(f"[POLYGON] (stub) stream {symbols} date={date_str} ingress={ingress} → {outdir}/{label}_*")

def _add_common(ap: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ap.add_argument("--symbols")
    ap.add_argument("--symbols-file")
    ap.add_argument("--date", required=True)
    ap.add_argument("--out", default=default_out("polygon"))
...
def main():
    top = argparse.ArgumentParser("Polygon Data CLI")
